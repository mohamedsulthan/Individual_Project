...
import ApplicationData from '../services/ApplicationsData';
...
const Cards = (props) => {
    ...
    const handleConfirmApply = async (data) => {
        try {
            const RegisterRequest = await ApplicationData.JobApplication(data);
            if (RegisterRequest.data.message === 'Event created successfully') {
                toast.success("Successfully Applied.", {
                    position: toast.POSITION.TOP_RIGHT,
                });
                const SendMail = await ApplicationData.SendMail(data);
